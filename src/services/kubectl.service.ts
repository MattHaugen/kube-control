...
function cleanseTerminalOutput(terminalOutput: object[]): object[] {
...
export default class KubectlService {
   ...
   getResource(resourceLabel: string): Promise<Array<object>> {
      return exec(`kubectl get ${resourceLabel}`)
         .then(result => {
            ...
            return cleanseTerminalOutput(stdout);
         })
         .catch(err => Promise.reject(err));
   }
   ...
   static getContexts(specificContext?: string): Promise<Array<object>> {
      ...
      const command = specificContext
         ? baseCommand.concat(` ${specificContext}`)
         : baseCommand;
      return exec(command)
         .then(result => {
   ...
   static getLogs(resource: string): Promise<string> {
      return new Promise(resolve => {
         const spawnProcess = spawn("kubectl", [
            "logs",
            `${resource}`,
            "--tail=100"
         ]);
